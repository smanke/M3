...
enum UpdateController {
    ...
    private static func confirmInstall(newVersion: String, current: String, allowSkip: Bool) -> ConfirmChoice {
        ...
        //
        // This app takes keyboard focus when the dialog opens, so a Return the user
        // meant for another app lands here. When "Update and Restart" was the default
        // button, that one keystroke installed an update from a dialog nobody had read
        // (reproduced 2026-09-13). "Not Now" is now first, so it owns Return. "Update
        // and Restart" has no key equivalent, and InstallConsent keeps it disabled
        // until the dialog has been on screen and in front for a moment, and records
        // the press itself.
        ...
        NSLog("M3 Tracker: update prompt response=\(response.rawValue) installPressed=\(consent.pressed)")
    ...
    ///
    /// The button starts disabled. It is enabled only after the dialog has been
    /// key, visible and unobscured, with the app active, for `delay` without a
    /// break, and it is disabled again the moment any of that stops being true.
    /// The press is recorded here rather than inferred from runModal's return
    /// value, so an install requires this button to have actually been pressed
    /// while enabled.
    @MainActor
